...
void IdRoleManager::manageRoles() {
    ...
    if (broadcastAddress[0] != 0x00)  sendIdTo(broadcastAddress);
    if (broadcastAddress1[0] != 0x00) sendIdTo(broadcastAddress1);
    if (broadcastAddress2[0] != 0x00) sendIdTo(broadcastAddress2);
    if (broadcastAddress3[0] != 0x00) sendIdTo(broadcastAddress3);
}
...
void IdRoleManager::manageTestRoles() {
    ...
    delay(1000); // Ensure ESP-NOW is ready
    if (broadcastAddress2[0] != 0x00) sendIdTo(broadcastAddress2);
    if (broadcastAddress4[0] != 0x00) sendIdTo(broadcastAddress4);
}
